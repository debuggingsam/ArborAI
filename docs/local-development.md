...
2. Run `npm install` (there are currently no third-party runtime dependencies).
3. Run `npm run dev` from the repository root. The API is available at `http://localhost:3001/health` and the web status page at `http://localhost:3000`.
...
Validation commands are `npm run lint`, `npm run typecheck`, `npm test`, and `npm run build`.
