...
test('rejects a node parent from another conversation', async () => {
  const db = { conversationNode: { findUnique: async () => ({ conversationId: 'other' }), create: async () => undefined } } as never;
  await assert.rejects(() => new ConversationRepository(db).createNode({ conversationId: 'current', parentId: 'parent', role: 'user', content: 'x', status: 'pending' }), /another conversation/);
});
